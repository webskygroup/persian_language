<?php
namespace Opencart\Admin\Model\Extension\PersianLanguage\Report;
/**
 * Class Sale
 *
...
class Sale extends \Opencart\System\Engine\Model {
	...
	public function getTotalOrdersByMonth(): array {
		...
		for ($i = 1; $i <= jdateEn('t'); $i++) {
			$cur_jdateEn = jdateEn('Y') . '-' . jdateEn('m') . '-' . $i;
			$date = jtg($cur_jdateEn);
			$order_data[jdateEn('j', strtotime($date))] = [
				'day'   => jdateEn('d', strtotime($date)),
				'total' => 0
			];
		}
		$first_jmdate = jdateEn('Y') . '-' . jdateEn('m') . '-1';
		$first_gmdate = jtg($first_jmdate);
		$query = $this->db->query("SELECT COUNT(*) AS total, `date_added` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > '0' AND DATE(`date_added`) >= '" . $this->db->escape($first_gmdate) . "' GROUP BY DATE(`date_added`)");
		foreach ($query->rows as $result) {
			$order_data[jdateEn('j', strtotime($result['date_added']))] = [
				'day'   => jdateEn('d', strtotime($result['date_added'])),
				'total' => $result['total']
			];
		}
		return $order_data;
	...
	$implode = [];
		...
		for ($i = 1; $i <= 12; $i++) {
			...
			$jyear = jdateEn('Y');
			$order_data[$jmonth] = [
				'month' => jdateEn('F', jmktime(0, 0, 0, $jmonth)),
				'total' => 0
			];
			$first_jmdate = jdateEn('Y') . '-' . $jmonth . '-1';
			$first_gmdate = jtg($first_jmdate);
			$last_jday = jdateEn('t', jmktime(12, 0, 0, $jmonth, 1, jdateEn('Y')));
			$last_gmdate = jdateEn('Y') . '-' . $jmonth . '-' . $last_jday;
			$query = $this->db->query("SELECT COUNT(*) AS total, `date_added` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > '0' AND DATE(`date_added`) >= '" . $this->db->escape($first_gmdate) . "' GROUP BY MONTH(`date_added`)");
			if ($query->num_rows) {
				foreach ($query->rows as $result) {
					$order_data[jdateEn('n', strtotime($result['date_added']))] = [
						'month' => jdateEn('F', strtotime($result['date_added'])),
						'total' => $result['total']
					];
				}
			}
