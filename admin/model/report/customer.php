...
class Customer extends \Opencart\System\Engine\Model {
	...
	public function getTotalCustomersByMonth(): array {
		...
		for ($i = 1; $i <= jdateEn('t'); $i++) {
			...
			$customer_data[jdateEn('j', strtotime($date))] = [
		...
		$query = $this->db->query("SELECT COUNT(*) AS `total`, `date_added` FROM `" . DB_PREFIX . "customer` WHERE DATE(`date_added`) >= '" . $this->db->escape($first_gmdate) . "' GROUP BY DATE(`date_added`)");
		foreach ($query->rows as $result) {
			$customer_data[jdateEn('j', strtotime($result['date_added']))] = [
	...
	$customer_data = [];
		for ($i = 1; $i <= 12; $i++) {
			...
			$query = $this->db->query("SELECT COUNT(*) AS `total`, `date_added` FROM `" . DB_PREFIX . "customer` WHERE DATE(`date_added`) >= '" . $this->db->escape($first_gmdate) . "' GROUP BY MONTH(`date_added`)");
			if ($query->num_rows) {
				foreach ($query->rows as $result) {
					$customer_data[jdateEn('n', strtotime($result['date_added']))] = [
