...
/* **********************Add By Mojtaba Moghani - Update In 2023/01/03********************** */
function jtg($jdate) {
	if ($jdate != '') {
...
function jtgcalc($date) {
    if ($date!='0000-00-00') {
		if ($date!='0000-00-00 00:00:00') {
			...
			$pattern = "/-/";
			list($jyear, $jmonth, $jday) = preg_split($pattern, $date); 
...
function gtj($date){
    if ($date!='0000-00-00') {
		...
		$pattern = "/-/";
		list($gyear, $gmonth, $gday) = preg_split($pattern, $date); 
		//list($jyear, $jmonth, $jday) = gregorian_to_jalali($gyear, $gmonth, $gday); 
		//$jdate = $jyear."-".$jmonth."-".$jday;
	$jdate = gregorian_to_jalali_persian($gyear, $gmonth, $gday, '-');//new
...
/*	F	*/
function jdateEn($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'en') {
//echo  $timestamp;
...
//echo  $timestamp;
  ...
  // echo convert($out);
  return ($tr_num != 'en') ? tr_num(convert($out), 'fa', '.') : convert($out);
}
...
function gregorian_to_jalali_persian($gy, $gm, $gd, $mod = '') {
   list($gy, $gm, $gd) = explode('_', tr_num($gy . '_' . $gm . '_' . $gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
  $g_d_m = array(0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334);
  $gy2 = ($gm > 2) ? ($gy + 1) : $gy;
  $days = 355666 + (365 * $gy) + ((int) (($gy2 + 3) / 4)) - ((int) (($gy2 + 99) / 100)) + ((int) (($gy2 + 399) / 400)) + $gd + $g_d_m[$gm - 1];
  ...
  return ($mod == '') ? array(convert($jy), convert($jm),convert( $jd)) : convert($jy) . convert($mod) . convert($jm) . convert($mod) . convert($jd);
}
/*	F	*/
function jalali_to_gregorian($jy, $jm, $jd, $mod = '') {
   list($jy, $jm, $jd) = explode('_', tr_num(convertEn($jy) . '_' . convertEn($jm) . '_' . convertEn($jd)));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
  $jy += 1595;
